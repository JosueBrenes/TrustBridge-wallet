...
interface CreateWalletFlowProps {
  onWalletCreated: (publicKey: string, secretKey: string, password: string) => void;
  onBack: () => void;
}

type FlowStep = 'password' | 'recovery-phrase' | 'confirm-phrase';

export function CreateWalletFlow({ onWalletCreated, onBack }: CreateWalletFlowProps) {
  ...
  const handleConfirmPhrase = () => {
    const isCorrect = selectedWords.every((word, index) => word === recoveryPhrase[index]);
    ...
    return (
      <Card className="w-full max-w-md mx-auto">
        <CardHeader className="text-center">
          ...
          <p className="text-sm text-muted-foreground">
            Your recovery phrase is the key to your account and is the only way to recover it. Keep it safe.
          </p>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-3">
            <p className="flex items-center space-x-2">
              ...
              <span>Your recovery phrase gives you full access to your wallet and funds</span>
            </p>
            <p className="flex items-center space-x-2">
              <span className="w-2 h-2 bg-yellow-500 rounded-full"></span>
              <span>If you forget your password, you can use the recovery phrase to access your wallet</span>
            </p>
            <p className="flex items-center space-x-2">
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
            <p className="text-sm text-yellow-700">
              No one from Stellar Foundation will ever ask for your recovery phrase
            </p>
          </div>
              ...
              <div className="grid grid-cols-3 gap-2 p-4 bg-gray-50 rounded-lg">
                {recoveryPhrase.map((word, index) => (
                  <div key={index} className="flex items-center space-x-2 text-sm">
                    <span className="text-gray-900 font-medium w-4">{index + 1}.</span>
          ...
          <Button variant="outline" onClick={() => setCurrentStep('password')} className="w-full">
            Back
          </Button>
        </CardContent>
      </Card>
    ...
    return (
      <Card className="w-full max-w-md mx-auto">
        ...
        <CardContent className="space-y-4">
          ...
          {confirmationError && (
            <div className="text-red-500 text-sm text-center">{confirmationError}</div>
          )}
          ...
          <Button variant="outline" onClick={() => setCurrentStep('recovery-phrase')} className="w-full">
            Back
          </Button>
        </CardContent>
      </Card>
